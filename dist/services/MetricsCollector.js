...
class MetricsCollector {
    ...
    async recordJobCompletion(jobType, durationMs) {
    ...
    async recordJobFailure(jobType, errorMessage) {
        await this.recordMetrics([
            {
    ...
    async recordApiCall(apiName, durationMs, success) {
        await this.recordMetrics([
            {
    ...
    async recordReviewsProcessed(count, platform) {
        await this.recordMetrics([
            {
    ...
    async recordWorkerStatus(status, currentJobs) {
        await this.recordMetrics([
            {
    ...
    async recordTimer(metricName, operation, tags) {
    ...
    async recordCounter(metricName, value = 1, tags) {
        await this.recordMetrics([
            {
                metric_name: metricName,
                metric_value: value,
    ...
    async recordGauge(metricName, value, unit, tags) {
        await this.recordMetrics([
            {
                metric_name: metricName,
                metric_value: value,
