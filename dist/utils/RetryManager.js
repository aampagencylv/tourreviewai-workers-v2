...
class RetryManager {
    ...
    async executeWithRetryAndResult(operation, options) {
        const startTime = Date.now();
        let attempts = 0;
        try {
    ...
    async retryDatabaseOperation(operation, maxAttempts = 3) {
    ...
    async retryDataForSEOCall(operation, maxAttempts = 5) {
        return this.executeWithRetry(operation, {
            maxAttempts,
    ...
    addJitter(delayMs, jitterPercent = 0.1) {
