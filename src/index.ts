...
import { ScalableJobProcessor } from './services/ScalableJobProcessor.js';
...
async function main() {
  ...
  const scalableProcessor = new ScalableJobProcessor(config, logger);
  ...
  app.get('/api/tripadvisor/progress/:syncJobId', async (req, res) => {
    try {
      ...
      const { data, error } = await (scalableProcessor as any).supabase
